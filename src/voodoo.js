...
const ERROR_DIVISIONS_BY_0 = "voodoo.js error: Division by 0";
const ERROR_IMPOSSIBLE = "voodoo.js error: Impossible";
...
function Evaluator(istr) {	
	...
	this.solve_oper = () => {
		...
		if (!voodoo.array_contains("cos,tan,sin,acos,atan,asin,abs".split(","), q)) b = this.ns.pop();
		switch (q) {
			case "+": 
				this.ns.push(a + b); 
				break;
			...
			case "tan": 
				...
				break;
			case "/": 
				if (a === 0) throw ERROR_DIVISIONS_BY_0;
				this.ns.push(b / a); 
				break;
			case "%": 
				if (a === 0) throw ERROR_DIVISIONS_BY_0;
		...
		if (o == ")") {
			while (this.os.length > 0) {
				if (voodoo.array_last(this.os) == "(") {
					this.os.pop();
					if (this.os.length && voodoo.array_contains("cos,tan,sin,acos,atan,asin,abs,max,min".split(","), voodoo.array_last(this.os)))
						this.solve_oper();	
					return;
				}
				this.solve_oper();	
	...
	this.is_const = () => {
		...
		if (t) {
			...
			switch (t[0]) {
				...
				default: throw ERROR_IMPOSSIBLE;
			}
			this.equation = this.equation.slice(t[0].length);
			return 1;
		}
...
// deep
function extend(a, b) {	
	if (is_undefined(a) || is_null(a)) return b;
	if (is_undefined(b) || is_null(b)) return a;
	for (const i in b) {
...
function Voodoo() {
	...
	this.unit_vector = () => {let l = 0; return this.each(v => {l += v * v;}).single(() => {l = voodoo.sqrt(l); assert(l !== 0, ERROR_DIVISIONS_BY_0);}).each(v => {return v / l;});};
	...
	this.divide = x => {assert(x !== 0, ERROR_DIVISIONS_BY_0); return this.each(v => {return v /= x;});};	
	this.mod = x => {assert(x !== 0, ERROR_DIVISIONS_BY_0); return this.each(v => {return v % x;});};	
	this.average = () => {let n = 0, l = 0; return this.each(v => {n += v; l++;}).single(() => {assert(l !== 0, ERROR_DIVISIONS_BY_0);}).set(n / l);};
...
extend(voodoo, {
    ...
    range: (a, b) => {var t = []; for (let i = a; i <= b; i++) t.push(i); return t;},
    between01: x => {return voodoo.max(0, voodoo.min(1, x));},
    random: (a = 0, b = 1) => {if (b < a) {[a, b] = [b, a];} return a + (b - a) * Math.random();},
    to_number: s => {return +s;},
    make_number: s => {			
	    let t = /^[\-]?\d*\.?\d+/g.exec(s);
	    return (t && t[0]) ? voodoo.to_number(t[0]) : 0;
    },
    own_property: (o, x) => {return voodoo.is_object(o) && o.hasOwnProperty(x) ? 1 : 0;},
    array_last: a => {return a.length ? a[a.length - 1] : a;},
    array_contains: (a, v) => {return a.indexOf(v) === -1 ? 0 : 1;},
    string_contains: (s, x) => {return s.indexOf(x) > -1 ? 1 : 0;},
    merge: (a, b) => {return a.concat(b);},
    time_since: time_since,

    get_date: (seconds = 0) => {var d = new Date(); d.setTime(d.getTime() + 1000 * seconds); return d;},
    date_to_string: d => {return d.toGMTString();},

    is_numeric: n => {return !isNaN(parseFloat(n)) && isFinite(n);}, // https://stackoverflow.com/a/9716488
    ...
    to_json: x => {return JSON.parse(x);},
    stringify_json: x=>{return JSON.stringify(x);},
    eval: s => {return (new Evaluator(s)).eval();},
    seconds: () => {return (new Date()).getSeconds();},
    minutes: () => {return (new Date()).getMinutes();},
    hours: () => {return (new Date()).getHours();},
    stringify_complex: c => {return `${c.re} ${c.im}i`;},
    delay: (f, ms = 100, errorf = error) => {var id = 0; new Promise((resolve, reject) => {id = window.setTimeout(resolve, ms);}).then(x => f(x)).catch(e => errorf()); return id;},

    ajax: opts => {
        let o = voodoo.extend({type: "GET", success: ()=>{}}, opts);
    ...
    delta: (a, b) => {return voodoo.abs(a) < b ? 1 : 0;},
    square: x => {return x * x;},
    cube: x => {return x * x * x;}
});
...
"floor,ceil,round,sin,cos,asin,acos,atan,pow,exp,sqrt".split(",").forEach(x=>{voodoo[x] = Math[x]; Voodoo.prototype[x] = function() {return this.each(v => {return voodoo[x](v);});};});
"abs,max,min".split(",").forEach(x => {Voodoo.prototype[x] = function() {return this.each(v => {return voodoo[x](v);});};});
"E,LN2,LN10,LOG2E,LOG10E,PI,SQRT1_2,SQRT2".split(",").forEach(x=>voodoo[x]=Math[x]);

if (!NDEBUG) pr(voodoo.ID);
